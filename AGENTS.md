...
*   **Reproductor Local Nativo**: `mpv` controlado vía sockets TCP a través del Named Pipe `\\.\pipe\mpv-socket` en Windows. Soporta WASAPI Exclusive, gapless audio y ReplayGain.
...
*   [`/backend`](file:///c:/Users/Sebas/level-player/backend): Servidor local en Node.js que maneja archivos, configuraciones y el reproductor nativo mpv.
...
| **usePlayer** / **PlayerProvider** | `context/PlayerContext.tsx` | Administra el estado de reproducción y se sincroniza con el backend (control local mpv vía SSE y fetch/POST) o usa Howler de fallback |
...
| **playerService** | `src/services/playerService.ts` | Controla la instancia local de `mpv.exe`, gestiona eventos por IPC socket, maneja transiciones, pre-cargas de audio y clientes SSE |
...
8.  **El Sistema Shuffle es Intocable (Dithering Distribuido):** El proyecto no usa un "Random" ingenuo. Utiliza un algoritmo de "Shuffle Distribuido" (al estilo Spotify) que agrupa las canciones por artista y las esparce matemáticamente en la cola para evitar repeticiones, pre-generando el array. Nunca reemplaces este sistema por `Math.random()` sobre la marcha, ya que rompería la lógica del botón "Atrás" (historial) del reproductor.
9.  **Soporte Multiformato de Carátulas Locales:** El endpoint `/api/cover` detecta carátulas en la carpeta de la canción buscando `cover.jpg`, `cover.png` y `cover.webp` en orden de prioridad. Evita forzar una sola extensión (.jpg) para no requerir renombramientos innecesarios.
10. **Motor de Sincronización de Letras (LRCLIB + Deezer Fuzzy):**
