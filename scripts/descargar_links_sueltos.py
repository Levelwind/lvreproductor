...
# -*- encoding: utf-8 -*-
"""
Descarga de enlaces sueltos (track, álbum, playlist) desde Tidal.
...
LINKS_FILE = r"c:\Users\Sebas\Music\scripts\links_sueltos.txt"
FAILED_LINKS_FILE = r"c:\Users\Sebas\Music\scripts\fallos_sueltos.txt"
MAX_RETRIES = 5
DELAY_BETWEEN = 2
...
def main():
    tracks = read_links()
    if not tracks:
        print("[!] No hay links sueltos listados para descargar. Saliendo.")
        return
    ...
    for i, url in enumerate(tracks, 1):
        print(f"\n{'='*60}")
        print(f"[{i}/{total}] Procesando Link: {url}")
        print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("  REPORTE FINAL DE LINKS SUELTOS")
    print(f"{'='*60}")
    ...
    with open(LINKS_FILE, "w", encoding="utf-8") as f:
        f.write("# Pega aquí los enlaces SUELTOS de Tidal (canciones, álbumes, playlists).\n")
        f.write("# Ejemplo: https://tidal.com/track/504160953\n")
